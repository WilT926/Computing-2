...
import { SVG_NS, configBase as importedConfigBase } from './catanconfig.js'; // Use importedConfigBase if needed for defaults
 ...
 */
CatanUI.setAttributes = function (el, attrs) {
 ...
 */
CatanUI.defineHexagonClipPath = function (svgBoardEl, currentConfig) {
    ...
    const points = [];
    const visualRadius = currentConfig.HEX_SIZE - currentConfig.HEX_VISUAL_OFFSET;
    for (let i = 0; i < 6; i++) {
        const angle = Math.PI / 180 * (60 * i - 30);
        points.push(`${(visualRadius * Math.cos(angle)).toFixed(3)},${(visualRadius * Math.sin(angle)).toFixed(3)}`);
    }
 ...
 */
CatanUI.renderIslandBackground = function (svgBoardEl, currentConfig) {
    ...
    const islandLayers = [
        { name: 'sand', avgRadius: coreIslandRadius * 1.2, fill: '#F0E68C', irregularity: 0.05, numPoints: 40 },
        { name: 'rocky', avgRadius: coreIslandRadius * 1.15, fill: '#A9A9A9', irregularity: 0.03, numPoints: 80 },
        { name: 'green', avgRadius: coreIslandRadius * 1.1, fill: '#556B2F', irregularity: 0.04, numPoints: 40 }
    ];
    islandLayers.forEach(layer => {
        const polygon = document.createElementNS(SVG_NS, 'polygon');
        const pointsStr = CatanLogic.createIrregularPolygon(centerX, centerY, layer.avgRadius, layer.numPoints, layer.irregularity);
        CatanUI.setAttributes(polygon, { points: pointsStr, fill: layer.fill, id: `background-${layer.name}` });
 ...
 */
CatanUI.renderBoard = function (svgBoardEl, currentConfig, shuffledTiles) {
    ...
    const centeringOffsetX = (currentConfig.VIEWBOX_WIDTH - boardNativeWidth) / 2 - minX;
    const centeringOffsetY = (currentConfig.VIEWBOX_HEIGHT - boardNativeHeight) / 2 - minY;
    ...
    shuffledTiles.forEach((tile, index) => {
        ...
        for (let i = 0; i < 6; i++) {
            const angle = Math.PI / 180 * (60 * i - 30);
            polygonPoints.push(`${(visualRadius * Math.cos(angle)).toFixed(3)},${(visualRadius * Math.sin(angle)).toFixed(3)}`);
        }
        CatanUI.setAttributes(hexagonPolygon, { points: polygonPoints.join(' '), class: 'svg-hexagon-polygon', fill: tile.color || '#ddc' });
        ...
        CatanUI.setAttributes(image, {
            x: (-imageDisplayWidth / 2).toFixed(3), y: (-imageDisplayHeight / 2).toFixed(3),
            width: imageDisplayWidth.toFixed(3), height: imageDisplayHeight.toFixed(3),
            href: currentConfig.imagesPath + tile.image,
            'clip-path': 'url(#hexagon-clip)', preserveAspectRatio: 'xMidYMid slice'
        });
        ...
        if (tile.name !== currentConfig.tileTypes.DESERT.name) {
            if (numberTokenIndex < currentConfig.numberTokenSequence.length) {
                const numberValue = currentConfig.numberTokenSequence[numberTokenIndex];
                const tokenGroup = document.createElementNS(SVG_NS, 'g');
                CatanUI.setAttributes(tokenGroup, { class: 'number-token-group' });
                const tokenRadius = currentConfig.HEX_SIZE * 0.35;
                const tokenCircle = document.createElementNS(SVG_NS, 'circle');
                CatanUI.setAttributes(tokenCircle, { cx: 0, cy: 0, r: tokenRadius, class: 'number-token-circle' });
                tokenGroup.appendChild(tokenCircle);
                const tokenText = document.createElementNS(SVG_NS, 'text');
                const textClass = (numberValue === 6 || numberValue === 8) ? 'number-token-text-red' : 'number-token-text';
                const fontSize = tokenRadius;
                CatanUI.setAttributes(tokenText, {
                    x: 0, y: 0, class: `number-token-text ${textClass}`,
                    'font-size': `${fontSize.toFixed(1)}`, 'dominant-baseline': 'middle', 'text-anchor': 'middle'
                });
        ...
        hexagonsData.push({ tile, axial: axialCoord, pixelCenter: { x: svgCenterX, y: svgCenterY }, element: group });
    });
    return hexagonsData;
}
 ...
 */
CatanUI.createInteractiveElements = function (svgBoardEl, currentConfig, hexagonsData) {
    ...
    hexagonsData.forEach(hexData => {
        const vertices = CatanLogic.getHexagonVerticesSVG(hexData.pixelCenter, currentConfig.HEX_SIZE);
        vertices.forEach((vertex, i) => {
            const vertexKey = `${vertex.x},${vertex.y}`;
            if (!uniqueVertices.has(vertexKey)) { uniqueVertices.set(vertexKey, vertex); }
            ...
            if (!uniqueEdges.has(edgeKey)) { uniqueEdges.set(edgeKey, { v1: vertex, v2: nextVertex }); }
        });
    });
    ...
    uniqueEdges.forEach(edge => {
        ...
        CatanUI.setAttributes(lineEl, {
            x1: edge.v1.x, y1: edge.v1.y, x2: edge.v2.x, y2: edge.v2.y,
            class: 'svg-interactive-line', 'stroke-width': currentConfig.LINE_THICKNESS
        });
    ...
    uniqueVertices.forEach(vertex => {
        ...
        CatanUI.setAttributes(circleEl, {
            cx: vertex.x, cy: vertex.y, r: currentConfig.POINT_RADIUS, class: 'svg-interactive-point'
        });
