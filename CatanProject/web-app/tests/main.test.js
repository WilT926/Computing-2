...
describe('Catan Logic Module (CatanLogic object tests)', function () {
    ...
    describe('CatanLogic.axialToPixel', function () {
        ...
        it('should correctly convert (1,0)', function () {
            ...
            const expectedY = 0;
            const delta = 0.001;
            if (Math.abs(result.x - expectedX) > delta || Math.abs(result.y - expectedY) > delta) {
                throw new Error(
    ...
    describe('CatanLogic.generateTiles', function () {
        ...
        const expectedTotalTiles = Object.values(testTileTypes).reduce((sum, type) => sum + type.count, 0);
        ...
        it('should generate correct counts for each tile type', function () {
            for (const typeKey in testTileTypes) {
                ...
                const count = tiles.filter(t => t.name === typeInfo.name).length;
    ...
    describe('CatanLogic.shuffleTiles', function () {
        it('should maintain all original elements and length after shuffle', function () {
            const originalTiles = [{id:1}, {id:2}, {id:3}, {id:4}, {id:5}];
            ...
            originalTiles.forEach(originalTile => {
                const found = shuffled.some(shuffledTile => R.equals(shuffledTile, originalTile));
                if (!found) {
                    throw new Error(
            ...
            if (originalTiles.length > 1 && R.equals(shuffled, originalTiles)) {
                 console.warn("CatanLogic.shuffleTiles produced the same order as original; this can happen randomly.");
            }
        });
    });
    ...
    describe('CatanLogic.createIrregularPolygon', function () {
        it('should return a string of points', function () {
            const pointsStr = CatanLogic.createIrregularPolygon(100, 100, 50, 10, 0.1);
            if (typeof pointsStr !== 'string') {
                throw new Error(`CatanLogic.createIrregularPolygon did not return a string. Got: ${typeof pointsStr}`);
            }
            if (pointsStr.length === 0 || !pointsStr.includes(',')) {
                 throw new Error(`CatanLogic.createIrregularPolygon returned an invalid points string: "${pointsStr}"`);
            }
        });

        it('should generate the specified number of points', function () {
            const numPoints = 12;
            const pointsStr = CatanLogic.createIrregularPolygon(0, 0, 30, numPoints, 0.2);
            const pointsArray = pointsStr.split(' ').filter(p => p.length > 0);
    ...
    describe('CatanLogic.getHexagonVerticesSVG', function() {
        ...
        it('should return an array of 6 vertices', function() {
            ...
            if (!Array.isArray(vertices) || vertices.length !== 6) {
                throw new Error(`Expected 6 vertices, got ${vertices ? vertices.length : 'not an array'}`);
            }
        });
        ...
        it('a vertex should have x and y properties', function() {
            ...
            if (typeof firstVertex.x !== 'number' || typeof firstVertex.y !== 'number') {
                throw new Error(`Vertex should have numeric x and y properties. Got: ${formatForError(firstVertex)}`);
            }
        });
        ...
        it('all vertices should be approximately hexSize distance from the center', function() {
            ...
            vertices.forEach((v, index) => {
                const dist = Math.sqrt(Math.pow(v.x - center.x, 2) + Math.pow(v.y - center.y, 2));
                if (Math.abs(dist - hexSize) > delta) {
                    throw new Error(`Vertex ${index} distance from center is wrong. Expected ~${hexSize}, Got: ${dist.toFixed(3)}`);
                }
            });
        });
    });
